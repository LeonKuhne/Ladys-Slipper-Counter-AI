class argHandler(dict):
    ...
    def setDefaults(self):
        ...
        self.define('annotation', '../pascal/VOCdevkit/ANN/',
        ...
        self.define('momentum', 0.0,
                    'applicable for rmsprop and momentum optimizers')
        ...
        self.define('lr', 1e-4, 'learning rate')
        self.define(
            'keep', 20, 'Number of most recent training results to save')
        self.define('batch', 16, 'batch size')
        self.define('epoch', 1000, 'number of epoch')
        self.define('save', 50, 'save checkpoint every ? training examples')
        self.define('demo', '', 'demo on webcam')
        self.define('queue', 1, 'process demo in batch')
        self.define('json', False,
                    'Outputs bounding box information in json format.')
        self.define('saveVideo', False,
                    'Records video from input video or camera')
        self.define(
        ...
        while i < len(args):
            ...
            if isinstance(self.get(argumentName), bool):
                if not (i + 1) >= len(args) and (args[i + 1].lower() != 'false' and args[i + 1].lower() != 'true') and not args[i + 1].startswith('--'):
                    print(
                        'ERROR - Expected boolean value (or no value) following argument: ' + args[i])
                    print('Try running flow --help')
                    exit()
                ...
                if isinstance(self[argumentName], float):
                    try:
                        ...
                        print(
                            'ERROR - Expected float for argument: ' + args[i])
                        print('Try running flow --help')
                        exit()
